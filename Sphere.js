function Sphere (gl, nLon, nLat, color) {
    ...
    this.render = function (gl, program, flatOrRound) {
        ...
        for (var i = 0; i < this.faces.length; i++) {
            ...
            var color = this.color.plus(center.plus(this.fnormals[i].times(0.5)));
    ...
    this.planeHit  = function (f, q, u) {
        var n = this.fnormals[f];
        var a = this.verts[this.faces[f][0]];
        return -n.dot(q.minus(a)) / (n.dot(u));//this calculates and returns s
    }
    ...
    this.contains = function (f, p) {
        ...
        for (var i = 0; i < face.length; i++) {
            ...
            var vi = this.verts[face[i]];
            var vj = this.verts[face[j]];
            if (n.dot(vi.minus(p).cross(vj.minus(p))) < 0) {
                return false
            };
        };
        return true;
    ...
    this.closestHit = function (q, u) {
        ...
        for (var f = 0; f < this.faces.length; f++) {
            var s = this.planeHit(f, q, u);
            if (s < 0) {
                continue
            }
            p = q.plus(u.times(s));
            if (this.contains(f, p) && s < sMin) {
                sMin = s
            }
        }
